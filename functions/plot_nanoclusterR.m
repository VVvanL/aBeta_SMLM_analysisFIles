function h = plot_nanoclusterR(xy_n0,xy_n1,ld0,ld1,roiEdges,aes) %#ok<*INUSD,*INUSL>
    ...
    h = figure;       
%     ax = axes; 
%     scatter(ax,xy(:,1),xy(:,2),aes.sz,'k','filled','MarkerFaceAlpha',0.25);
%     axis ij; axis(normEdges); pbaspect([aspect 1 1]); xlabel('x(nm)'); ylabel('y(nm)');
        
    ax0 = axes;    
    scatter(ax0,xy_n0(:,1),xy_n0(:,2),aes.sz,ld0,'filled','MarkerFaceAlpha',0.75);
    axis ij; axis(normEdges); pbaspect([aspect 1 1]); colormap(ax0,aes.cmap0);
    xlabel('x(nm)'); ylabel('y(nm)');
    
    ax1 = axes;
    scatter(ax1,xy_n1(:,1),xy_n1(:,2),aes.sz,ld1,'filled','MarkerFaceAlpha',0.75);
    axis ij; axis(normEdges); pbaspect([aspect 1 1]); colormap(ax1,aes.cmap1);
    
    linkaxes([ax0,ax1]); ax1.Visible = 'off';
